...
from integracoes.a3soft.client import login_obter_token, enviar_nf as a3_enviar_nf, enviar_ticket as a3_enviar_ticket
...
def webhook():
    ...
    def get_or_fetch_a3soft_token(numero: str, conversas: dict) -> str | None:
        # tenta reaproveitar
        tok = conversas.get(numero, {}).get("a3soft_token")
        if tok:
            return tok
        # senão, loga com as creds do .env
        res = login_obter_token()
        if res.get("ok") and res.get("token"):
            token = res["token"]
            conversas.setdefault(numero, {})["a3soft_token"] = token
            return token
        return None
