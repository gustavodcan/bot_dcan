...
def webhook():
    try:
        ...
        if tipo != "ReceivedCallback":
            print(f"Ignorando tipo: {tipo}")
            return jsonify(status="ignorado")

        print(f"Estado atual da conversa com {numero}: {estado}")
        print(f"Mensagem recebida: {texto_recebido}")

        if not estado:
            enviar_mensagem(numero, "👋 Olá! Tudo bem? Sou o bot de tickets da DCAN Transportes.\nPor acaso você seria motorista em viagem pela DCAN?", buttons=[
                {"id": "sim", "text": "Sim"},
                {"id": "nao", "text": "Não"}
            ])
        ...
        if estado == "aguardando_confirmacao_motorista":
            if texto_recebido in ['sim', 's']:
                enviar_mensagem(numero, "✅ Perfeito! Para qual cliente a descarga foi realizada? ArcelorMittal, Gerdau, Raízen ou ProActiva?", buttons=[
                ...
                enviar_mensagem(numero, "❓ Por favor, responda apenas SIM ou NÃO.", buttons=[
                    {"id": "sim", "text": "Sim"},
                    {"id": "nao", "text": "Não"}
                ])
            ...
            if "image" in data and data["image"].get("mimeType", "").startswith("image/"):
                ...
                try:
                    ...
                    if img_res.status_code == 200:
                        ...
                        print(f"Erro HTTP ao baixar imagem: {img_res.status_code}")
                ...
                enviar_mensagem(numero, msg, buttons=[
                    {"id": "sim", "text": "Sim"},
                    {"id": "nao", "text": "Não"}
                ])
